...
```{r}
# Recode to binary classification
data.all$Clm_Flag <- as.factor(ifelse(data.all$Clm_Count >= 1, "Y", "N"))
...
For insurance datasets involving claims, it is common for the positive (i.e. $Clm\_Flag =$ "Y") responses to represent a minority of the overall exposures. If left unchecked, this can lead to weak discriminating power between the positive and negative classes during the modeling phase.
...
data.training$Exp_weights <- data.training$Exp_weights *
  ifelse(data.training$Clm_Flag == "Y", samp.scale, 1)  # Upsample the minority class

library(dplyr)
data.training %>%  # Show table to check that oversampling worked
...
* Cost-complexity _pruning_, a post-processing step that removes selected nodes based on some criteria.
...
While cross-validation is a reliable technique in the machine learning toolkit, it is important to keep in mind that this is only as useful as the available data, and there may be unobservable factors governing the true population dynamics. In particular, if the dataset is sparsely populated or lacking in signaling features, it is still possible to overfit to noise. There may also be cases where it is not appropriate to partition (e.g. imbalanced datasets result in degenerative folds) or cases where the partitioned sets underfit the data; however, this should be detectable across folds.

> __Knowledge Check:__ The above graph plots the average error rate produced over the ``xval = 5`` folds for possible choices of the _complexity parameter_, defined as the minimum impurity reduction needed for the learning algorithm to continue splitting nodes. What is a reasonable choice for the value of the complexity parameter, and how would the depth of the tree change?

### Predicting New Observations

A predictive modeling problem typically involves taking a position on what might happen in the future based on historical data, in contrast to descriptive or prescriptive analytics. To assess whether the model generalizes well to as-yet unseen data, the testing set is utilized. By holding out a portion of data until the evaluation stage, models can be fairly compared against each other in what approximates a live setting.

```{r}
pred.tree <- predict(model.tree, data.testing, type = "prob")  # Predict claim incidence
pred.tree <- pred.tree[, "Y"] * data.testing$Exp_weights  # Multiply by exposure
pred.tree <- as.factor(ifelse(pred.tree <= 0.5, "N", "Y"))  # Coerce to binary response

# Create confusion matrix
library(e1071)
confusionMatrix(data = pred.tree, reference = data.testing$Clm_Flag, positive = "Y")
```
...
Coerce <- function(model, data, threshold) {
  predictions <- predict(model, data)
  predictions <- predictions$predictions[, "Y"] * data$Exp_weights
  predictions <- as.factor(ifelse(predictions <= threshold, "N", "Y"))
  predictions
}

pred.bag <- Coerce(model.bag, data.testing, 0.5)
confusionMatrix(pred.bag, data.testing$Clm_Flag, positive = "Y")
```

> __Knowledge Check:__ Examine the arguments of the ``Coerce`` function. How do the performance measures change when the function evaluates the training data instead of the testing data? What happens to $TPR$ and $TNR$ as the probability threshold increases beyond 0.5?

### Ranking Variable Importance

A common criticism of ensemble methods is that their oft-superior predictive capabilities come at the expense of transparency. Whereas linear models can effortlessly link two or more variables via coefficients, non-parametric models are inherently fragmented and abstractions are often strained. However, this can be mitigated with feature importance rankings. In particular, the number of times a feature is used to partition across trees can be enumerated.

```{r}
# Extract variable importance rankings
var.imp <- data.frame(Feature = names(model.bag$variable.importance),
...
ggplot(var.imp, aes(x = reorder(Feature, Splits), y = Splits)) +  # Plot variable importance
  ...
  labs(x = "Feature", y = "Mean Decrease in Impurity") +
  coord_flip()
```

The bar chart above ranks variables according to their mean decrease in impurity, which is the impurity reduction that results from splitting a node, averaged across all trees and weighted in proportion to the number of observations that the variable splits, expressed as a percentage.

> __Knowledge Check:__ How is it possible for the variable $VAgeCat$ to have a mean decrease in impurity of over 100%?
...
pred.forest <- Coerce(model.forest, data.testing, 0.5)
confusionMatrix(pred.forest, data.testing$Clm_Flag, positive = "Y")
```
...
for(i in 1:nrow(hyper.grid)){
  model.tuned <- ranger(Clm_Flag ~ SexInsured + Female + VehicleType + PC + NCD + AgeCat + VAgeCat,
                        ...
                        case.weights = data.training$Exp_weights,
                        splitrule = 'gini')
  pred.tuned1 <- Coerce(model.tuned, data.training, 0.5)
  hyper.grid$train.error[i] <- 1 - confusionMatrix(pred.tuned1,
                                                   data.training$Clm_Flag,
                                                   positive = "Y")$overall[[1]]
  pred.tuned2 <- Coerce(model.tuned, data.testing, 0.5)
  hyper.grid$test.error[i] <- 1 - confusionMatrix(pred.tuned2,
                                                  data.testing$Clm_Flag,
                                                  positive = "Y")$overall[[1]]
}

hyper.grid[order(hyper.grid$train.error)[1:5],]  # Display instances of minimum training error

# Select candidate model based on tuning grid
model.tuned <- ranger(Clm_Flag ~ SexInsured + Female + VehicleType + PC + NCD + AgeCat + VAgeCat,
                      data = data.training,
                      num.trees = 20, 
                      mtry = 5,
...
pred.tuned <- predict(model.tuned, data.testing)$predictions[, "Y"] * data.testing$Exp_weights
```
...
There is a natural tendency to introduce complexity into a model for the sake of increasing predictive accuracy, but this can result in inconsistent errors (i.e. variance) when exposed to new observations, owing to the inclusion of irrelevant features. On the other hand, overly simplified model structures tend to generate consistent errors (i.e. bias), owing to the exclusion of relevant features.

Collectively, this phenomenon is referred to as the __bias-variance tradeoff__, which acknowledges that expected loss, while not totally eliminable, can at least be minimized within some interior bounds. A plot of training versus testing error illustrates how finely-tuned models often struggle when applied to novel data. 
...
ggplot(subset(hyper.grid, mtry == 5 & sample.fraction == 0.4)) +
  ...
  labs(x = "Number of Trees", y = "Error Rate",
       title = "Random Forest with 5 of 7 Features, 40% Resampling") +
  scale_x_continuous(breaks = seq(0, 20, 2)) +
  scale_color_manual(labels = c("Testing Error", "Training Error"),
                     values = c('test' = "orange", 'train' = "blue"))
```

> __Knowledge Check:__ In the graph above, what causes the testing error to initially fall, and then gradually rise? Why is testing error generally higher than training error?
...
In cases where the modeled output is a probability, as is the case in the models established so far, the response can be coerced to a binary outcome using a threshold value. An __ROC curve__ plots the true positive rate against the false positive rate for all such thresholds. The area under this curve measures the closeness to a perfect classification model.

```{r message=F}
library(ROCR)
pred <- prediction(pred.tuned * data.testing$Exp_weights, data.testing$Clm_Flag)
...
ggplot(roc.curve) +
  ...
  ggtitle(paste("Area Under ROC Curve:",
...
PlotLift <- function(df, var, exposure, response, fit){  # One-way lift chart
  ...
  liftplot <- ggplot(dframe, aes(x = Factor, y = SumWeight, group = 1)) +
    ...
    geom_line(data = dframe.melt,
              aes(x = Factor, y = value, color = variable, group = variable), size = 1.5) +
    theme(legend.position="bottom", legend.direction="horizontal", legend.title = element_blank()) +
    scale_color_manual(labels = c("Reference", "Prediction"),
...
data.lift$YHat <- as.integer(ifelse(pred.tuned <= 0.5, 0, 1))
data.lift$Clm_Flag <- as.integer(ifelse(data.lift$Clm_Flag == "Y", 1, 0)) # Recode target variable

PlotLift(data.lift, 'AgeCat', 'Exp_weights', 'Clm_Flag', 'YHat')
```

## Crunch Residuals

Other useful diagnostics involve residuals which, in the case of classification problems, are difficult to interpret. One technique to overcome this challenge is to "crunch" the residuals by aggregating exposures into equally sized bins, averaging the predicted and observed responses within each cell, and comparing the fitted values to their residuals.
...
Crunch <- function(df, fitted, target, weight, size = 50) {  # Function to compute crunch residuals
  ...
  z <- z[sample(nrow(z)), ]  # Shuffle rows and assign class based on weight
  test <- setDT(z)[, as.list(colMeans(.SD)), by = ceiling(cumsum(z[, wt])/sum(z[, wt])*size)]
  test$res <- test$y - test$yhat
  qplot(y = test$res, x = test$yhat,
        ylab = "Residual", xlab = "Fitted Value", main = "Crunched Residuals")
}
